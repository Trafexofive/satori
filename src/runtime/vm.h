...
// Chunk — a sequence of bytecode instructions with parallel
//          source-line tracking (like Clang/Lua/Python).
//   code[i]      — the bytecode
//   lines[i]     — source line for code[i] (or 0 if no info)
typedef struct {
  u8 *code;
  int *lines;       // Source line for each byte, parallel to code[]
  int count;
  int capacity;
  Value *constants;
  int constant_count;
...
typedef struct VM {
  ...
  // Source context for runtime error display
...
// Set source context for error display
void vm_set_source(VM *vm, SourceInfo *source_info);

// Get the source line for the current instruction
