...
typedef struct {
    ...
    // Current line being compiled (from AST node)
    int current_line;
...
static void emit_byte(Compiler *c, u8 byte) {
    chunk_write(c->chunk, byte, c->current_line);
}
...
static void patch_jump(Compiler *c, int offset) {
    ...
    if (jump > 0xffff) {
        const char *notes[] = {
            "note: try simplifying this expression"
        };
...
static void compile_call(Compiler *c, AstNode *node) {
    ...
    if (call->callee->type == AST_MEMBER_ACCESS) {
        ...
        if (member->object->type == AST_IDENTIFIER) {
            ...
            char full_name[256];
            snprintf(full_name, sizeof(full_name), "%s.%s", obj->name,
                     member->member);
...
static void compile_node(Compiler *c, AstNode *node) {
    ...
    case AST_ASSIGNMENT: {
        ...
        if (slot >= 0) {
            ...
            const char *notes[] = {
                "note: variables must be declared with `let` before assignment"
            };
            diag_emit(LEVEL_ERROR, c->source ? c->source->file_path : NULL,
                      node->line, node->column, 0,
    ...
    case AST_MEMBER_ACCESS: {
        const char *notes[] = {
            "note: member access (`.name`) is only valid as part of a function call"
        };
        diag_emit(LEVEL_ERROR, c->source ? c->source->file_path : NULL,
                  node->line, node->column, 0,
                  "Member access must be used in a call",
                  c->source, notes, 1);
        c->had_error = true;
        break;
    }
    ...
    case AST_INT_LITERAL: {
        int constant =
            make_constant(c, value_make_int(node->as.int_literal.value));
        emit_bytes(c, OP_CONSTANT, constant);
        break;
    }
    ...
    default: {
        diag_emit(LEVEL_ERROR, c->source ? c->source->file_path : NULL,
                  node->line, node->column, 0,
                  "Unknown AST node type in codegen",
                  c->source, NULL, 0);
        c->had_error = true;
        break;
    }
